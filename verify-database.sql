...
-- 1. Check if exam questions exist
...
SELECT QuestionNumber, 
       SUBSTRING(QuestionText, 1, 80) AS QuestionPreview,
       MaxScore, 
       Subject 
FROM ExamQuestions 
WHERE ExamId = 'SAMPLE-EXAM-001' 
...
SELECT TOP 10
    ...
    Status,
    StatusCode,
    DATEDIFF(MINUTE, EvaluationStartedAt, GETUTCDATE()) AS MinutesStuck,
    ErrorMessage
FROM WrittenSubmissions
WHERE Status = 2 -- Evaluating
  AND EvaluationStartedAt IS NOT NULL
ORDER BY EvaluationStartedAt DESC;

-- 4. If questions are missing, run these INSERTs:
/*
...
-- 5. Verify questions after insert
SELECT QuestionNumber, QuestionText, MaxScore, Subject 
FROM ExamQuestions 
WHERE ExamId = 'SAMPLE-EXAM-001' 
ORDER BY QuestionNumber;
